    ...
    for eatery in static_json["eateries"]:
        new_eatery = CampusEateryType(
            ...
            operating_hours=parse_static_op_hours(
                eatery.get("operatingHours", []), parse_dining_items(eatery), eatery.get("datesClosed", [])
            ),
    ...
    for i in range(NUM_DAYS_STORED_IN_DB):
        ...
        for dates in dates_closed:  # check if dates_closed contains new_date
            ...
            new_operating_hours.append(
                OperatingHoursType(date=new_date.isoformat(), events=parse_events(new_events, new_date.isoformat()))
            )
